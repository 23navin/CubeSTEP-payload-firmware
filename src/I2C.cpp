...
I2cCore::I2cCore(){
    ...
    ESP_LOGV(TAG, "Device Address: %02x", DEVICE_ADDR);
...
void I2cCore::init(){
    ...
    if (err != ESP_OK) {
        ...
        ESP_LOGD(TAG, "Device Mode: %i", config.mode);
        ESP_LOGD(TAG, "Device Address: %02x", DEVICE_ADDR);
    }
...
void I2cCore::write_one_byte(uint8_t tx_data){
    ...
    ESP_LOGI(TAG, "Data Response: %02x", tx_data);
...
void I2cCore::write_one_byte_raw(uint8_t tx_data){
    ...
    if(tx_data != 0xAA) ESP_LOGI(TAG, "Byte Sent: %02x", tx_data);
...
void I2cCore::write_string(std::string tx_data){
    ...
    ESP_LOGD(TAG, "Data Response: %x", tx_buffer);
    ...
    //         tx_buffer[i] = tx_data.at(i);
    //         ESP_LOGD(TAG, "buffer[%i]< %02X", i, tx_buffer[i]);
    //     }

    //     int j = 0;
    //     for(int i = TX_SIZE_MAX; i < tx_size; i++) {
    //         tx_buffer2[j] = tx_data.at(i);
    //         ESP_LOGD(TAG, "buffer2[%i]< %02X", j, tx_buffer2[j]);
    ...
    //     ESP_LOGD(TAG, "writing i2c");
    //     ESP_LOGD(TAG, "Data Response: %X", tx_buffer);
    //     ESP_LOGD(TAG, "Data Response: %X", tx_buffer2);
    //     write_one_byte_raw(START_BYTE);
...
bool I2cCore::check_for_message(){
    ...
    if(bytes_read > 0) {
        ESP_LOGI(TAG, "Message Received: %#02x", *operation);
        ...
        ESP_LOGD(TAG, "OpCode 0x%02x", *operation);
        ...
        if(paramater_size > 0) {
            ...
            ESP_LOGI(TAG, "Parameter received: %x", parameter);
        }
...
void I2cCore::install_handler(uint8_t opcode, void (*i2c_handler_ptr)(uint32_t)){
    ...
    ESP_LOGD(TAG, "%#02x installed with handler", opcode_list[opcode_counter], handler_list[opcode_counter]);
}
...
void I2cCore::install_handler(uint8_t opcode){
    ...
    ESP_LOGD(TAG, "%#02x installed without handler", opcode_ignore_list[opcode_ignore_counter]);
}
...
void I2cCore::find_handler(uint8_t opcode, uint32_t parameter){
    ...
    if(opcode_id > -1){
        ESP_LOGD(TAG, "Calling handler for OpCode 0x%02X", opcode_id);
